 ...
 * @description Form wizard kuisioner BSAN — 37 pertanyaan dengan skip logic & auto-save draft
 * @tables pertanyaan_survey, jawaban_survey, responden_survey, modul_bsan, alur_tema
 * @queries database/queries/auth.sql → Get user profile
      ...
      <div className="space-y-6">
        ...
        {isModalOpen && createPortal(
          <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/75 p-4 animate-fade-in">
            <div className="w-full max-w-md rounded-2xl bg-surface p-6 shadow-2xl border border-border space-y-4 animate-scale-in">
              <div className="flex items-center justify-between border-b border-border pb-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1">
                    <CustomSelect
                      label="Tipe Isian"
                      options={[
                        { value: 'essay', label: 'Essai Narasi' },
                        { value: 'radio', label: 'Pilihan Ganda' },
                        { value: 'rating', label: 'Skala Rating 1-5' },
                        { value: 'text', label: 'Teks Singkat' },
                      ]}
                      value={modalType}
                      onChange={(val) => setModalType(val as any)}
                    />
                  </div>
                  ...
                  <div className="space-y-1">
                    <CustomSelect
                      ...
                      options={[
                        { value: 'true', label: 'Wajib Diisi (*)' },
                        { value: 'false', label: 'Opsional' },
                      ]}
                      value={modalReq ? 'true' : 'false'}
                      onChange={(val) => setModalReq(val === 'true')}
                    />
                  </div>
                </div>
