...
import type { SELJawaban, SELObservasiSession, SELSchoolScore } from '../lib/data-source';
import {
  SEL_INDIKATORS, SEL_DIMENSI_ORDER, SEL_DIMENSI_LABEL,
  SEL_SKOR_LABEL, getIndikatorsByFilter,
} from '../lib/sel-indicators';
import type { SELDimensi, SELSkor } from '../lib/sel-indicators';
import {
        ...
        <div className="rounded-2xl bg-surface border border-border shadow-card p-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="space-y-1">
              <label className="font-bold text-text-secondary uppercase text-[10px]">Kabupaten / Kota *</label>
            ...
            <div className="space-y-1">
              <label className="font-bold text-text-secondary uppercase text-[10px]">Kecamatan *</label>
          ...
          <div className="border-t border-border pt-4">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
              ...
              <div className="space-y-1">
                <label className="font-bold text-text-secondary uppercase text-[10px]">JK Guru</label>
                <select value={guruJK} onChange={e => setGuruJK(e.target.value as 'L'|'P')}
                  className="w-full rounded-xl border border-border bg-bg px-3 py-2.5 text-text-primary focus:border-primary focus:outline-none">
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-surface rounded-2xl shadow-2xl border border-border w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <h4 className="text-xs font-bold text-text-primary">Ringkasan Jawaban per Dimensi</h4>
            {SEL_DIMENSI_ORDER.map(d => {
              const inds = getIndikatorsByFilter({ dimensi: d });
              const guruInds = inds.filter(i => i.subjek === 'guru');
              const muridInds = inds.filter(i => i.subjek === 'murid');
              const avgSkor = (indList: typeof inds) => {
                const vals = indList.map(i => jawabanMap[i.id]).filter((v): v is SELSkor => v != null);
                return vals.length ? (vals.reduce((a, b) => a + b, 0) / vals.length) : 0;
              };
              const guruAvg = avgSkor(guruInds);
              const muridAvg = avgSkor(muridInds);
              return (
                <div key={d} className="p-3 bg-bg rounded-xl border border-border/50">
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-xs font-bold text-text-primary">{SEL_DIMENSI_LABEL[d]}</span>
                    <SkorBadge skor={Math.round(((guruAvg + muridAvg) / 2) * 10) / 10} />
                  </div>
                  <div className="flex gap-4 text-[11px] text-text-secondary">
                    <span>Guru: <strong className="text-primary">{guruAvg.toFixed(1)}</strong></span>
                    <span>Murid: <strong className="text-accent">{muridAvg.toFixed(1)}</strong></span>
                  </div>
                </div>
              );
            })}
          ...
          {selStats && (
            <div className="flex flex-wrap gap-2">
              {[
                ...
                <div key={item.label} className="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-2.5 text-center">
...
export default function ObservasiSEL({ userRole = 'school' }: { userRole?: 'admin' | 'school' }) {
