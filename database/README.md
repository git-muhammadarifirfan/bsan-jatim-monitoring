...
|---|---|---|---|
| **Super Admin** | `super_admin` | *Global (NULL)* | Akses penuh seluruh server, database, dan log audit. |
| **Admin Dinas Provinsi** | `admin_pusat` | *Global (NULL)* | Monitoring makro 38 Kab/Kota Jawa Timur & ekspor laporan provinsi. |
| **Admin Dinas Kab/Kota** | `admin_kabupaten` | `kabupaten_id` | Terkunci di 1 Kabupaten/Kota (misal Kab. Sidoarjo). |
| **Pengawas / Korwil Kec.** | `admin_kecamatan` | `kecamatan_id` | Penilik / Korwil yang mengisi/memantau Observasi SEL di 1 Kecamatan. |
| **Operator / Kepala Sekolah** | `operator_sekolah` | `sekolah_id` | Terkunci di 1 Sekolah (NPSN) untuk pengisian instrumen survei mandiri. |
| **Stakeholder Eksternal** | `viewer` | *Opsional* | Read-only statistik dan grafik untuk mitra/peneliti. |
...
| Log In Identifier (Email / NPSN) | Password | Peran (Role) | Akses |
|---|---|---|---|
| `admin@sidoarjo.go.id` | `admin` | `admin_pusat` / `admin_kabupaten` | Dashboard Dinas Pendidikan Sidoarjo |
| `20512345` / `sekolah@sch.id` | `sekolah` | `operator_sekolah` | Dashboard & Form Survei SDN Candi 1 Sidoarjo |
